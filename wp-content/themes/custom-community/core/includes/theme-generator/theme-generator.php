...
class CC_Theme_Generator{
	 ...
	 * function replaced by ICAD: now generates three column spread of multiple lists
...
/*			$args = array(
			...
			echo '<div class="default-homepage-last-posts">';
 ...
 */
			...
			/*
			foreach (array($cap->default_homepage_last_posts_cat1, $cap->default_homepage_last_posts_cat2, $cap->default_homepage_last_posts_cat3) as $args['category_name']) {
			...
			*/
			...
			foreach (array($cap->default_homepage_last_posts_cat1, $cap->default_homepage_last_posts_cat2, $cap->default_homepage_last_posts_cat3) as $args['category_name']) {
				remove_all_filters('posts_orderby');
				$catObj = get_category_by_slug( $args['category_name'] );
				$catName = $catObj->name;
				$tmp .= '<div class="boxgrid_column" style="float:left;width:222">';
				$tmp .= '<h2>'.$catName.'</h2>';
				query_posts($args);
				if (have_posts()) {
					$thePath = array();
					...
					while (have_posts()) : the_post();
						...
						if ($is_first) {
							$tmp .= '<div class="boxgrid captionfull" style="background: transparent; -moz-background-clip: border; -moz-background-origin: padding; -moz-background-inline-policy: continuous; " title="'. $catName.'">';
							$tmp .= '<a href="'. get_permalink().'" title="'. $catName.'"><img src="'.$thePath[0].'" /></a>';
							$tmp .= '<div class="cover boxcaption">';
							$tmp .= '<h3><a href="'. get_permalink().'" title="'. get_the_title().'">'. get_the_title().'</a></h3>';
							$tmp .= '<p><a href="'. get_permalink().'" title="'. get_the_title().'">'.substr(get_the_excerpt(), 0, 100).'...</a></p>';
							$tmp .= '</div>';
							$tmp .= '</div><!--boxgrid-->';
							$tmp .= '<h3>recent posts</h3>';
							$tmp .= '<div class="boxgrid_postlink">';
							$tmp .= '<a href="'. get_permalink().'" title="'. get_the_title().'">'. get_the_title().'</a>';
							$tmp .= '</div>';
							$is_first = false;
						}
						else {
				...
				$tmp .= '<div class="boxgrid_postlink"><strong><a href="'. site_url('/category/'. $args['category_name']). '">view all posts</a></strong></div>';
				$tmp .= '</div><!--boxgrid_column-->'; 
			}
			...
			foreach (array($cap->default_homepage_last_posts_cat1, $cap->default_homepage_last_posts_cat2, $cap->default_homepage_last_posts_cat3) as $args['category_name']) {
				...
				$i = 0;
				$tmp .= '<div class="boxgrid_column" style="float:left;width:222">';
				foreach ($all_comments as $a_comment) {
					$comm_post_id = $a_comment->comment_post_ID;
					if (!in_category($args['category_name'], $comm_post_id)) continue;
					$comm_id = $a_comment->comment_ID;
					$comm_extract = get_comment_excerpt($comm_id);
					if (strlen($comm_extract) > 24) {
						$comm_extract = substr($comm_extract, 0, 27);
						$comm_extract = substr($comm_extract, 0, strrpos($comm_extract, ' ', -1));
						$comm_extract .= '...';
					}
					$comm_tmp .= '<div class="boxgrid_commlink">';
					$comm_tmp .= '<a href="'. get_permalink($comm_post_id).'#comment-'. $comm_id. '">'. $comm_extract. '</a>';
					$comm_tmp .= '</div>';
					$i++;
					if ($i > $comm_max) break;
				}
				if ($comm_tmp) $tmp .= '<h3>recent comments</h3>'. $comm_tmp;
				$tmp .= '</div><!--boxgrid_column-->'; 
			}
			$tmp .='<div class="clear"></div>';
			print '<div class="list-posts-all">'.$tmp.'</div>&nbsp;';
			echo '</div>';
