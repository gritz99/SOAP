...
// ============================== new function by ICAD
function get_category_dropdown() {
	$cats = get_categories(array('orderby'=>'names'));
	$return = array(__('', 'cc'));
	foreach ($cats as $cat) {
		array_push($return, __($cat->slug, 'cc'));
	}
	return $return;
}
...
function cap_get_options() {
	...
	new Group (__("General",'cc'), "general",
		...
		new DropdownOption(
			...
			__("Display latest post for three categories (select categories below). <br /> ",'cc'),
		...
		new DropdownOption(
			__("Category 1",'cc'), 
			__("Choose first category. <br />",'cc'),
			"default_homepage_last_posts_cat1", 
		...
		new DropdownOption(
			__("Category 2",'cc'), 
			__("Choose first category. <br />",'cc'),
			"default_homepage_last_posts_cat2", 
			get_category_dropdown(),
			'', 
			false),	
		new DropdownOption(
			__("Category 3",'cc'), 
			__("Choose first category. <br />",'cc'),
			"default_homepage_last_posts_cat3", 
			get_category_dropdown(),
			'', 
			false),	
		new DropdownOption(
	...
	new Group (__("BuddyPress",'cc'), "buddypress",
		...
		new TextOption(
			...
			"")/*,
		new TextOption(
			__("Registration Whitelist",'cc'), 
			__("Enter e-mail filters that you would like to whitelist for registration. Users who register with e-mail addresses that match these filters will be able to register without administrative approval. Use an asterisk (*) for a wildcard (e.g., *@somewhere.com), and separate the filters with a comma.",'cc'), 
			"bp_registration_whitelist", 
			"")*/
		)
		),
